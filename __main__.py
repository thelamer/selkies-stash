...
def main():
    ...
    parser.add_argument('--render-mode',
                        default=os.environ.get(
                            'SELKIES_RENDER_MODE', 'gstreamer'),
                        help='Rendering mode: gstreamer (default), jpeg-datachannel')
    ...
    app = GSTWebRTCApp(stun_servers, turn_servers, audio_channels, curr_fps, args.encoder, gpu_id, curr_video_bitrate, curr_audio_bitrate, keyframe_distance, congestion_control, video_packetloss_percent, audio_packetloss_percent, render_mode=render_mode)
    ...
    # Screen capture module setup
    jpeg_queue = None
    capture_module = None
    stripe_callback = None
    is_capturing = False
    if render_mode == "jpeg-datachannel":
        capture_settings = CaptureSettings()
        capture_settings.capture_width = 1920 # You might want to make these configurable via args later
        capture_settings.capture_height = 1080
        capture_settings.capture_x = 0
        capture_settings.capture_y = 0
        capture_settings.target_fps = float(args.framerate)
    ...
    def on_session_handler(session_peer_id, meta=None):
        ...
        if str(session_peer_id) == str(peer_id):
            ...
            if render_mode == "gstreamer": # Existing GStreamer pipeline
                logger.info("starting video pipeline (gstreamer mode)")
                app.start_pipeline()
            elif render_mode == "jpeg-datachannel": # New JPEG datachannel mode
                logger.info("starting jpeg datachannel capture")
                if capture_module and stripe_callback:
                    app.start_jpeg_datachannel_stream(capture_module, capture_settings, stripe_callback, jpeg_queue)
                else:
                    logger.error("Capture module or callback not initialized for jpeg-datachannel mode!")
    ...
    try:
        ...
        app.stop_pipeline()
        if capture_module and render_mode == "jpeg-datachannel":
            capture_module.stop_capture()
            del capture_module
...
def py_stripe_callback(result_ptr, user_data):
    ...
    if is_capturing and jpeg_queue and loop:
        result = result_ptr.contents
        if result.data:
            data_bytes = ctypes.cast(
                result.data, ctypes.POINTER(ctypes.c_ubyte * result.size)
            ).contents
            stripe_data = bytes(data_bytes)
            asyncio.run_coroutine_threadsafe(jpeg_queue.put(stripe_data), loop)
        else:
            print("py_stripe_callback received empty stripe data.")
